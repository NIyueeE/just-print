...
- PCL-XL 队列的双面依赖上游 Ghostscript 的逐页正反面标记：Debian 12 的
  Ghostscript 10.0.0 会因页计数被「向上取偶」而把第 2 页起标成正面（表现为
  「选了双面却单面出纸」），v0.3.7 起镜像基于 Debian 13（10.05.1）已不再触发。
  仍在使用旧镜像的环境请升级镜像，而不要改用后处理过滤器。
