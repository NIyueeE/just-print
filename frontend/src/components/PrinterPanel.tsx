      ...
      {isNumericOption(spec) ? (
        <input
          ...
          onInput={(event) => {
            ...
            if (next !== '') {
              dispatch({ type: 'options/set', key: optionKey, value: next })
            }
          }}
        />
      ) : (
...
export function PrinterPanel() {
  ...
  function openConfirmation(): void {
    ...
    dispatch({
      type: 'print/confirm',
      pending: {
        payload: { file_id: upload.result.id, printer_id: selected.id, options },
        printerName: selected.name,
        fileName: upload.result.name,
        summary: summarizeOptions(selected, options).map((row) => ({
          label: row.label,
          value: row.value,
        })),
      },
